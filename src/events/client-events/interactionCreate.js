	...
	async execute(interaction) {
		...
		try {
			...
			if (interaction.replied || interaction.deferred) {
				await interaction.followUp({ content: 'There was an error while executing this command! Interaction Create', flags: MessageFlags.Ephemeral });
			}
			else {
				await interaction.reply({ content: 'There was an error while executing this command! Interaction Create', flags: MessageFlags.Ephemeral });
			}
		}
	},
};